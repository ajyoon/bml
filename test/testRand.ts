...
function testGeneratorFunctionTypeAndRange(
  ...
  typeValidationFunction: ValidateNumberKind,
  min: number,
  max: number
) {
  for (let i = 0; i < 100; i++) {
...
describe("setRandomSeed", function () {
  ...
  it("produces stable results forever", function () {
    ...
    expect(hash).toBe(
      "f2f381924630531a5e188f5cdbd110b90f90b796f7daf0dcf402070e8d46ae80"
    );
  });
});
...
describe("weightedChoose", function () {
  ...
  it("errors when given no weights", function () {
    expect(() => rand.weightedChoose([])).toThrow(
      "Cannot perform weighted choice where the given weights have a sum of zero"
    );

...
import { EvalBindingError, EvalDisabledError } from "../src/errors";
...
describe("render", function () {
  ...
  it("executes nested forks", function () {
    let testString =
      "foo {(bar {(nested branch), (other nested branch)}), (biz)}";
    expect(render(testString, null)).toEqual("Foo bar other nested branch\n");
  });
...
`;
  ...
  it("supports re-executing references", function () {
    let testString =
      "foo {id: (bar), (biz)} {@!id} {@!id} {@!id} {@!id} {@!id}";
    expect(render(testString, null)).toEqual("Foo bar biz bar biz biz biz\n");
  });
...
`;
  ...
  it("supports re-executing set forks", function () {
    ...
    expect(render(testString, null)).toEqual("B D A C\n");
  });
  ...
  it("resets weights on exhausted sets", function () {
    let testString =
      "{#$id: (A), (B), (C), (D)} {@!id} {@!id} {@!id} {@!id} {@!id}";
    expect(render(testString, null)).toEqual(" B D A C C\n");
...
`;
  ...
  it("Allows disabling eval execution", function () {
    ...
    expect(() => render(testString, { allowEval: false })).toThrow(
      EvalDisabledError
    );
  });
...
{@foo: 0 -> (bar), 1 -> (biz)}
`;
    expect(render(testString, null)).toEqual("Y\nbiz\n");
  });

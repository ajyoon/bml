...
- **BREAKING CHANGE**: The underlying pseudorandom number generator has been
  changed from the `seedrandom` default of `ARC4` to `xor4096`. The new
  generator is faster and has a significantly longer period of 2^4096-2^32.
  This is a breaking change because BML documents with fixed seeds will have
  different outputs with 0.2.0 as compared to earlier versions. No other
...
- _Internal change_: the repo has been migrated to Typescript. All
...
_skipped due to bad release from Typescript build headaches_
